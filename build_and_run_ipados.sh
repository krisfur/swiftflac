...
# Pick a booted iPad simulator if there is one, otherwise the first available iPad.
UDID=$(xcrun simctl list devices booted | grep -m1 "iPad" | grep -oE '[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}' || true)
if [ -z "$UDID" ]; then
    UDID=$(xcrun simctl list devices available | grep -m1 "iPad" | grep -oE '[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}')
fi
